...
GF.screens['receptionist-memberships']=function(){document.getElementById('main-content').innerHTML=`${GF.screenHeader('Đăng ký gói & Thẻ','Quy trình tiếp nhận và kích hoạt gói cho hội viên','<button class="btn btn-primary" onclick="GF.toast.success(\'Đã tạo hồ sơ đăng ký mới\')"><i class="bi bi-plus-circle"></i>Tạo đăng ký</button>')}<div class="payment-steps"><div class="payment-step completed"><span class="payment-step-num"><i class="bi bi-check"></i></span><span class="payment-step-label">Chọn hội viên</span></div><span class="payment-step-line completed"></span><div class="payment-step active"><span class="payment-step-num">2</span><span class="payment-step-label">Chọn gói</span></div><span class="payment-step-line"></span><div class="payment-step"><span class="payment-step-num">3</span><span class="payment-step-label">Thanh toán</span></div><span class="payment-step-line"></span><div class="payment-step"><span class="payment-step-num">4</span><span class="payment-step-label">Kích hoạt thẻ</span></div></div><div class="row g-4"><div class="col-lg-8"><div class="card p-4"><h5>Hội viên đã chọn</h5><div class="d-flex align-items-center gap-3 mt-3">${GF.renderAvatar('Nguyễn Văn An',48)}<div><strong>Nguyễn Văn An</strong><p>MV001 · 0901 234 567 · an.nguyen@gmail.com</p></div><button class="btn btn-sm btn-outline-secondary" style="margin-left:auto">Thay đổi</button></div><div class="divider"></div><h5 class="mb-3">Chọn gói đăng ký</h5><div class="row">${GFData.packages.map((p,i)=>`<div class="col-md-4"><button class="payment-method-card ${i===1?'selected':''}" style="width:100%;height:100%;flex-direction:column;align-items:flex-start"><strong>${p.name}</strong><span class="text-blue fw-bold">${GF.fmt.currency(p.price)}</span><span class="text-xs text-muted">${p.duration}</span></button></div>`).join('')}</div></div></div><div class="col-lg-4"><div class="card p-4"><h5>Tóm tắt đăng ký</h5><div class="divider"></div><div class="d-flex justify-content-between mb-3"><span>Gói Premium</span><strong>2.850.000 ₫</strong></div><div class="d-flex justify-content-between mb-3"><span>Phí kích hoạt thẻ</span><strong>Miễn phí</strong></div><div class="d-flex justify-content-between mb-4"><span>Hiệu lực</span><strong>3 tháng</strong></div><button class="btn btn-primary btn-lg w-full" onclick="GF.navigate('receptionist-payments')">Tiếp tục thanh toán<i class="bi bi-arrow-right"></i></button></div></div></div>`};
GF.screens['receptionist-payments']=function(){const rows=[['HD26071401','Nguyễn Văn An','Premium 3 tháng','2.850.000 ₫','Chờ thu'],['HD26071404','Phạm Thị Mai','Gói PT 10 buổi','3.500.000 ₫','Chờ thu'],['HD26071328','Hoàng Văn Đức','Basic 1 tháng','650.000 ₫','Quá hạn']];document.getElementById('main-content').innerHTML=`${GF.screenHeader('Thanh toán cơ bản','Tiếp nhận khoản thu và chuyển giao dịch cho quầy thu ngân','<button class="btn btn-primary" onclick="GF.toast.success(\'Đã tạo yêu cầu thanh toán\')"><i class="bi bi-plus-circle"></i>Tạo yêu cầu</button>')}<div class="alert alert-info mb-4">Lễ tân có thể ghi nhận tiền mặt hoặc tạo yêu cầu thanh toán; hoàn tiền và hủy giao dịch do Thu ngân xử lý.</div><div class="card"><div class="card-header"><input class="form-control" style="max-width:320px" placeholder="Tìm hội viên hoặc hóa đơn..."><select class="form-select" style="max-width:180px"><option>Chờ thanh toán</option><option>Đã thanh toán</option></select></div><div class="table-responsive"><table class="table"><thead><tr><th>Mã hóa đơn</th><th>Hội viên</th><th>Nội dung</th><th>Số tiền</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>${rows.map((x,i)=>`<tr><td><strong class="text-blue">${x[0]}</strong></td><td>${x[1]}</td><td>${x[2]}</td><td><strong>${x[3]}</strong></td><td><span class="badge ${i===2?'bg-danger-subtle text-danger':'bg-warning-subtle text-warning'}">${x[4]}</span></td><td><button class="btn btn-sm btn-primary" onclick="GF.toast.success('Đã ghi nhận thanh toán ${x[0]}')"><i class="bi bi-cash"></i>Thu tiền</button></td></tr>`).join('')}</tbody></table></div></div>`};
GF.screens['receptionist-support']=function(){const tickets=[['HT041','Nguyễn Văn An','Không nhận được email hóa đơn','Hóa đơn','Cao','Đang xử lý'],['HT040','Trần Thị Bình','Cần đổi lịch PT ngày 16/07','Lịch PT','Trung bình','Đã tiếp nhận'],['HT039','Phạm Thị Dung','Hỏi thủ tục bảo lưu gói','Gói tập','Thấp','Chờ phản hồi'],['HT038','Lê Minh Cường','QR check-in không quét được','Điểm danh','Cao','Hoàn thành']];GF.adminTablePage({title:'Hỗ trợ khách hàng',subtitle:'Tiếp nhận và theo dõi yêu cầu hỗ trợ hội viên',action:'<button class="btn btn-primary" onclick="GF.toast.success(\'Đã mở phiếu hỗ trợ mới\')"><i class="bi bi-plus-circle"></i>Tạo phiếu hỗ trợ</button>',stats:`<div class="col-md-3">${GF.stat('bi-inbox','#2563eb','8','Yêu cầu đang mở','3 yêu cầu mới')}</div><div class="col-md-3">${GF.stat('bi-hourglass','#f59e0b','5','Đang xử lý','Trung bình 22 phút')}</div><div class="col-md-3">${GF.stat('bi-check-circle','#10b981','34','Đã hoàn thành','Trong tuần này')}</div><div class="col-md-3">${GF.stat('bi-star','#8b5cf6','4,8/5','Mức hài lòng','Từ 26 đánh giá')}</div>`,heads:['Mã phiếu','Hội viên','Nội dung','Nhóm','Ưu tiên','Trạng thái','Thao tác'],rows:tickets.map(x=>`<tr><td><strong class="text-blue">${x[0]}</strong></td><td>${x[1]}</td><td>${x[2]}</td><td>${x[3]}</td><td>${x[4]}</td><td>${x[5]}</td><td><button class="btn btn-sm btn-outline-primary"><i class="bi bi-chat-dots"></i>Phản hồi</button></td></tr>`)})};
